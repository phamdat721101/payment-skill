...
| "should I review my defi loan rollover / morpho rollover risk / lending position rollover" | `defi_rollover_decision_engine` (read-only Morpho risk research with provenance and non-executable intent; no calldata, signing, or dispatch) |
...
| "analyze my borrow position / inspect lending health / pinned DeFi position scan" | `analyze_borrow_position` (block-pinned, read-only Dolomite/Morpho/Aave/Silo/Pendle state with provenance and explicit partial data) |
| "backtest my rollover strategy / model a DeFi price shock / replay borrow-position risk" | `backtest_rollover_strategy` (requires an archive RPC; modelled transitions only, never calldata or execution) |
